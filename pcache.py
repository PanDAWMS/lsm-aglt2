...
import sys, os, errno, fcntl, stat, time, getopt, re
import subprocess
import signal
...
#       self.hysterisis = 0.9
    ...
    def main(self, args):
        ...
        if ( os.path.exists(cache_file) ) :
            ...
            if ( (exit_status == 0) or (exit_status == 2) ) : self.finish()
    ...
    def pcache_copy_in(self):
        ...
        while ( retry <= self.max_retries ) :
          ...
          if ( retry > 0 ) : self.log(INFO, "do_transfer: retry %s", retry)
...
#       self.log(INFO, '%s', d)
    ...
    def do_transfer(self):
        ...
        if ( copy_output ) : print '%s' % copy_output
...
# +SPM May 24, 2011.  Add logging to syslog
        try:
            syslog.openlog()
            syslog.syslog("%s\n" % msg )
